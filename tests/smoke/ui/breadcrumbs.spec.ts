...
test.describe('Breadcrumbs Verification', () => {
    ...
    const CATEGORY_PAGE = `${BASE_URL}/smartphones/apple-iphone`; // Adjust based on actual category
    const PRODUCT_PAGE = `${BASE_URL}/smartphones/apple-iphone/apple-iphone-16-pro-128gb-black-titanium`; // Adjust based on actual product

    const BREADCRUMB_SELECTOR = '.breadcrumb';
    const BREADCRUMB_ITEMS_SELECTOR = '.breadcrumb li';
    const BREADCRUMB_LINKS_SELECTOR = '.breadcrumb li a';
    ...
    test('Verify breadcrumbs are visible on category page and clickable', async ({page}) => {
        ...
        const breadcrumbs = page.locator(BREADCRUMB_SELECTOR);
        await expect(breadcrumbs).toBeVisible();
        console.log('✅ Breadcrumbs are visible on the category page.');

        // Verify breadcrumb structure
        const breadcrumbItems = breadcrumbs.locator(BREADCRUMB_ITEMS_SELECTOR);
        ...
        const homeBreadcrumb = breadcrumbs.locator(BREADCRUMB_LINKS_SELECTOR).first();
    ...
    test('Verify breadcrumbs are visible on product page and reflect correct navigation path', async ({page}) => {
        ...
        const categoryBreadcrumb = breadcrumbItems.nth(1).locator('a');
